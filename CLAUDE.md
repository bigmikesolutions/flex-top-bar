...
- **Primary plugin:** `plugins/top-bar` — Vue 3 + Vite (admin + frontend), PHP REST API, Pinia, PHPUnit + Vitest + Playwright.
- **Tests:** From repo root: `npm run test` runs the **top-bar** workspace (Vitest); `npm run test:e2e` runs **Playwright** (`playwright.config.ts`, `e2e/`). PHPUnit: `composer test` (see README).
...
- REST routes: `includes/class-api.php`, namespace **`top-bar/v1`**. Keep payloads aligned with `plugins/top-bar/src/api/client.ts`.
- Options/normalization: `includes/class-options.php`; bar shape must stay consistent with `plugins/top-bar/src/types`.

---

## Top Bar Vue (`plugins/top-bar/src/`)
